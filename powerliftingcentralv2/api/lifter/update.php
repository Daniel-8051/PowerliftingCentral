...
if (!isset($_SERVER['PHP_AUTH_USER'])) {
    ...
    if (($_SERVER['PHP_AUTH_USER'] == 'admin' and ($_SERVER['PHP_AUTH_PW'] == 'password123'))) {
        ...
        header('Access-Control-Allow-Methods: PUT'); // allows put 
        ...
        // MeetCountry
        $meetcountry = $data['meet_country_name'];
        ...
        // sql query
        $sql = "UPDATE uk_ireland_powerlifting SET name='$name', sex='$sex', equipment='$equipment', age='$age', age_class='$ageclass', division='$division', bodyweight='$bodyweightkg', weight_class='$weightclasskg', 
        best_squat='$bestsquat', best_bench='$bestbench', best_deadlift='$bestdeadlift', total='$totalkg', place='$place', wilks='$wilks', ipf_points='$ipfpoints', tested='$tested', country='$country', federation='$federation', 
        date='$dateFormatted', meet_country='$meetcountry', meet_name='$meetname' WHERE lifter_id ='$lifter_id' ";
        ...
        if ($result) {
            echo json_encode(
                array('message' => 'Lifter updated')
            );
        } else {
            echo json_encode(
                array('message' => 'Lifter not updated')
            );
        }
    } else {
        header("WWW-Authenticate: Basic realm='Private Area'");
